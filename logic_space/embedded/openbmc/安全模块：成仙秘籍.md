...
patch 文件出现在 ${S}/patches/my_dts_change.patch

cp ${S}/patches/my_dts_change.patch meta-myboard/recipes-kernel/linux/files/
```
...
需要配合[quilt](#开发工具之quilt)进行手动补丁

### 开发工具之devtool

[Using the devtool command-line tool](https://docs.yoctoproject.org/dev/dev-manual/devtool.html)

典型开发流程

```
devtool modify sourcode.c

cd build/workspace/sources

devtool build

devtool finish：自动生成patch+bbappend
```
...
[Using Virtual Providers](https://docs.yoctoproject.org/dev-manual/new-recipe.html#using-virtual-providers)

```
Yocto支持同一个功能有多种实现方案，比如：
- 内核可以是linux-yocto/linux-mainline/linux-fslc等
- C库可以是glibc或musl
- EGL可以是mesa或ivante
如果每个地方都写死具体配方名，那么当你切换内核时，所有引用了它的地方都得改一遍，非常脆弱

virtual/就是解决这个问题的抽象层

    virtual/kernel 代表“系统内核”这个抽象需求
    具体的配方通过 PROVIDES += "virtual/kernel" 声明自己能提供这个抽象
    然后在配置文件中通过 PREFERRED_PROVIDER_virtual/kernel = "linux-yocto" 来选择实际使用哪个配方

这样一来，所有依赖内核的地方只需要写 DEPENDS = "virtual/kernel" 或 bitbake -c devshell virtual/kernel，不需要关心底层具体是哪个内核——切换内核时只需改一行配置，其他地方自动跟着变。
```
...
备注：使用virtual/kernel进行内核修改，详情见virtual provider
...
```
提取源码但不进入工作流
devtool extract phosphor-modbus /path/to/output
看完清理掉workspace状态
devtool reset phosphor-modbus
```
